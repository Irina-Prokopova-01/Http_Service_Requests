...
def test_retrieve_url_request(api_client, user):
    ...
    url_instance = URLrequest.objects.create(original_url="http://example.com", short_id="abc123")

    response = api_client.get(reverse('shorturl:url-retrieve', kwargs={'short_id': url_instance.short_id}))
...
def test_retrieve_nonexistent_url_request(api_client, user):
    ...
    response = api_client.get(reverse('shorturl:url-retrieve', kwargs={'short_id': 'nonexistent'}))
